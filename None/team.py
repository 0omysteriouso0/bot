...
async def huntcode(self, ctx):
  ...
  #damage
    ...
    n = (random.random()+1)/2
    damage = int((self.monDef/(self.monDef+self.Atk)* (n+0.5))* self.monDef)
    ...
    if nowHp <= 0:
      ...
      if self.monName == "雷電棉花糖雲" and "《成就》冷血殺手" not in self.achievement:
        ...
        embed.add_field(name=f"損失了{damage}點生命值",
                        value=f"目前生命: {nowHp}",inline=False)
        embed.add_field(name=f"獲得了{self.monExp+increase}點經驗值",
                        value=f"目前exp: {self.exp}", inline=False)
        embed.add_field(name=f"獲得了{getmoney}元",
                        value=f"目前金錢: {money}", inline=False)
        await ctx.reply(embed = embed)
        hunted = self.hunted.get(self.monName) + 1 if self.hunted.get(self.monName)!=None else 1
      ...
      #Reproduction huntRest
        rpg.update_one({"id":ctx.author.id}, {"$set": {
            "money":money,
            "nowHp":nowHp,
            "exp":self.exp,
        ...
        while self.exp>=self.lv*20:
          ...
          else:await ctx.channel.send(embed=content)
...
async def explorecode(self, ctx):
      ...
      #rest
        ...
        if seconds < self.advenrest[1]:
          await ctx.reply(f"精疲力盡的{self.name}還想休息{int(self.advenrest[1] - seconds)}秒")
          return 
      #Treasure
        #Money
        moneyBag=random.choice([
          {"名稱":'一小堆金幣',"金錢":random.randint(40,100)},
          {"名稱":'一袋錢',"金錢":random.randint(150,300)},
          {"名稱":'一小堆金幣',"金錢":random.randint(40,100)},
          {"名稱":'一袋錢',"金錢":random.randint(150,300)},
          {"名稱":"滿箱子的黃金","金錢":random.randint(70,100)*10}])
        self.chestMoney=f"{moneyBag['名稱']} 共{moneyBag['金錢']}元"
        #寶石
        self.gemstone=random.choice(["紅","黃","綠","藍","紫","七彩夢幻","靈魂"])+"寶石"
        #武器
        ...
        money_num = 2
        goodTreasure=[self.chestMoney]*money_num +[areaMaterial]*2 + [self.getWeapon] + ["技能卡包"]*2 + [self.gemstone]

        treasure=[random.choice(goodTreasure),random.choice(goodTreasure),random.choice(badTreasure)]
      #Increase
        #n is damageIncrease
        n=random.random()+1
        #choice what Events
      ...
      #AdvEvents
       ...
       #notHappend
        if Events < 1:
            ...
            rpg.update_one({"id":ctx.author.id},{"$set":{"探險休息":(datetime.datetime.now(), 25)}})
       #getSomething
        elif Events <= 5:
        ...
        #goodTreasure
          if getTreasure in goodTreasure:
              ...
              if getTreasure in goodTreasure[:money_num]:
                  ...
                  rpg.update_one({"id":ctx.author.id}, {"$set":{"money":money}})
              #else
              else:
                  Core.getSomething(self, rpg.find({"id": ctx.author.id}), getTreasure)
              ...
              rpg.update_one({"id":ctx.author.id},{"$set":{"探險休息":(datetime.datetime.now(), 45)}})
        #badTreasure
              ...
              #alive
                ...
                await ctx.channel.send(f"但你還是從他身上得到了{int(boxMoney)}元\n目前生命{int(nowHp)}")
                money= self.money+int(boxMoney)
                rpg.update_one({"id":ctx.author.id},{"$set":{
                  "money":money ,
                  "探險休息":(datetime.datetime.now(), 35)}})
       #Raid
        elif Events <= 6:
            await ctx.reply(f"你遭到{self.monName}的突襲")
            nowHp = self.nowHp - int(self.monAtk/(self.monAtk+self.Def)*n*self.monAtk)
            rpg.update_one({"id":ctx.author.id}, {"$set":{"nowHp":nowHp}})
            #dead
            if nowHp <= 0:
              text = Core.dead(self,self.monName)
            ...
            #alive
                ...
                await ctx.channel.send(f"損失了{int(self.monAtk/(self.monAtk+self.Def)*n*self.monAtk)}點生命值\n目前生命{int(nowHp)}")
            rpg.update_one({"id":ctx.author.id},{"$set":{"探險休息":(datetime.datetime.now(), 30)}})
...
#check
  ...
  if self.nowHp==self.maxHp:
    ...
    return
  healMoney = int((self.maxHp-self.nowHp)*1.5 *(0.7+self.nowHp/self.maxHp))
  rpg.update_one({"id":ctx.author.id}, {"$inc":{"money":-healMoney}})
  rpg.update_one({"id":ctx.author.id}, {"$set":{"nowHp":self.maxHp}})
  await ctx.reply(f"{self.name}你花了{int(healMoney)}元回滿了血量\n(maxHp: {self.maxHp})")
  return
...
#check
  ...
  elif rpg.find_one({"id":ctx.author.id})["nowSta"]<=0:
    await ctx.reply(f"{self.name}因打工而累倒了")
    return
    
  rpg.update_one({"id":ctx.author.id},
        {"$push":{"狀態":'打工'},"$set":
         {"useSta":0, "打工時間": datetime.datetime.now()}})
